...
class CSVUploadView(APIView):
    ...
    def __add_gigachat_description(self, row):
        messages = [
            SystemMessage(
                content="Ты - эксперт по рекламе c 10 летним опытом. Ты понимаешь почему данная рекламная компания полезна."
                        "И какую рекламу лучше предложить для размещения."
            ),
            HumanMessage(content=f"Опишии рекламную компанию. Вот такие данные по ней:"
